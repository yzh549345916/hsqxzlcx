...
namespace TelerikMvcApp3
{
    ...
    public class 数据库处理
    {
        ...
        public List<SKViewModel> 获取指定时间资料名统计信息(DateTime sDate, DateTime eDate, string dataName)
        {
            List<SKViewModel> mylists = new List<SKViewModel>();
            using (SqlConnection mycon = new SqlConnection(connStr))
            {
                ...
                {
                    mycon.Open(); //打开
                    string sql = $"select * from 入库个数统计信息 where 表名 = '{dataName}' and 时间>='{sDate}' and 时间<='{eDate}'"; //SQL查询语句 (Name,StationID,Date)。按照数据库中的表的字段顺序保存
                    SqlCommand sqlman = new SqlCommand(sql, mycon);
                    SqlDataReader sqlreader = sqlman.ExecuteReader();
